jest.mock('../../../lib/handlers/users/userUpdatedHandlerV1');
jest.mock('../../../lib/handlers/users/sendWSUserUpdatedV1');
jest.mock('./../../../lib/handlers/utils');

const { mockHandler, getDefaultConfig, getLoggerMock } = require('./../../testUtils');
const { getAllApplicationRequiringNotification } = require('./../../../lib/handlers/utils');
const userUpdatedHandlerV1 = require('../../../lib/handlers/users/userUpdatedHandlerV1');
const sendWSUserUpdatedV1 = require('../../../lib/handlers/users/sendWSUserUpdatedV1');
const { register } = require('../../../lib/handlers/users');
...
describe('when registering user handlers', () => {
  beforeEach(() => {
    getAllApplicationRequiringNotification.mockReset().mockReturnValue(applications);

    userUpdatedHandlerV1.getHandler.mockReset().mockReturnValue(userUpdatedHandlerV1Instance);

    sendWSUserUpdatedV1.getHandler.mockReset().mockImplementation((config, logger, application) => {
  ...
  it('then it should register user updated handler', async () => {
    ...
    expect(userUpdatedHandlerV1.getHandler).toHaveBeenCalledWith(config, logger);
  });
  ...
  it('then it should register send ws user updated handler for each application that requires notification', async () => {
    ...
    expect(sendWSUserUpdatedV1.getHandler).toHaveBeenCalledWith(config, logger, applications[0]);
    expect(sendWSUserUpdatedV1.getHandler).toHaveBeenCalledWith(config, logger, applications[1]);
  });
});