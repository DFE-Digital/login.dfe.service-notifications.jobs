  ...
  request(rurl, data, callback, exheaders, exoptions) {
    ...
    asyncRetry(async () => {
      ...
      if (!response.headers['content-type'].startsWith('text/xml')
        && !response.headers['content-type'].startsWith('application/soap+xml')
        && !response.headers['content-type'].startsWith('Multipart/Related')) {
...
class SecureAccessWebServiceClient {
  ...
  async provisionUser(action, saUserId, saUsername, firstName, lastName, emailAddress, organisationId, wsAccountStatusCode, establishmentUrn, localAuthorityCode, groupUpdates) {
    const message = this._provisionUserFormatter.getProvisionUserSoapMessage(this._targetNamespace, action, saUserId, saUsername, firstName, lastName, emailAddress, organisationId, wsAccountStatusCode, establishmentUrn, localAuthorityCode, groupUpdates);
    if (this._username) {
  ...
  async provisionOrganisation(action, establishmentDfeNumber, establishmentUrn, groupUid, localAuthorityCode, orgEdubaseTypeCode, organisationId, organisationName, organisationTypeCode, wsOrganisationStatusCode) {
    const message = this._provisionOrganisationFormatter.getProvisionOrganisationSoapMessage(this._targetNamespace, action, establishmentDfeNumber, establishmentUrn, groupUid, localAuthorityCode, orgEdubaseTypeCode, organisationId, organisationName, organisationTypeCode, wsOrganisationStatusCode);
    if (this._username) {
      message.setUsernamePassword(this._username, this._password).setTimestamp(60);
    }
    if (this._requireAddressing) {
