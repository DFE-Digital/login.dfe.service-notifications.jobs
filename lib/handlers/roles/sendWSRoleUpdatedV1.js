...
const getWebServiceClient = async (application, correlationId) => {
  ...
  if (!applicationClient) {
    ...
    if (application.provisionGroupFormatterType) {
      secureAccessWebServiceClient.setProvisionGroupFormatter(application.provisionGroupFormatterType);
    }
...
const getApplicationDetails = async (applicationsClient, applicationId) => {
  ...
  return {
    ...
    provisionGroupFormatterType: application.relyingParty.params.wsProvisionGroupFormatterType,
  }
};
