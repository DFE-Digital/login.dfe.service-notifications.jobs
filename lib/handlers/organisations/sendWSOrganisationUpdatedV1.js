...
const sendUpdatedUserToApplication = async (action, organisation, application, config, correlationId) => {
  try {
    ...
    let wsOrganisationStatusCode = null;
    if (organisation.status.id === 1) {
      wsOrganisationStatusCode = 'OPEN';
    } else if (organisation.status.id === 2) {
      wsOrganisationStatusCode = 'CLOSED';
    } else if (organisation.status.id === 3) {
      wsOrganisationStatusCode = 'CLOSED_BUT_ACTIVE';
    } else if (organisation.status.id === 4) {
      wsOrganisationStatusCode = 'PROPOSED_TO_OPEN';
    }
    await secureAccessWebServiceClient.provisionOrganisation(action, establishmentDfeNumber, organisation.urn, organisation.uid, localAuthorityCode, typeId, organisation.legacyId, organisation.name, organisation.category.id, wsOrganisationStatusCode)
  } catch (e) {
