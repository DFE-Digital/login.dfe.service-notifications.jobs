...
const sendUpdatedUserToApplication = async (action, organisation, application, config, correlationId) => {
  try {
    ...
    await secureAccessWebServiceClient.provisionOrganisation(action, establishmentDfeNumber, organisation.urn, organisation.uid, localAuthorityCode, typeId, organisation.legacyId, organisation.name, organisation.category.id, organisation.status.id)
  } catch (e) {
