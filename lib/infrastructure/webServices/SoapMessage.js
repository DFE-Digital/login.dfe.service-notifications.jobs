    ...
    if (value === null) {
      ...
      if (isArray) {
        for (let i = 0; i < value.length; i += 1) {
          xml += expandTreeToXml(value[i]);
        }
      } else if (isDate) {
        xml += value.toISOString();
      } else if (isObject) {
        xml += expandTreeToXml(value);
      } else {
        xml += value;
      }
      xml += `</${tag}>`;
...
class SoapMessage {
  ...
  toXmlString() {
    const now = new Date();
    let xml = `<soapenv:Envelope xmlns:soapenv="http://schemas.xmlsoap.org/soap/envelope/" xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" xmlns:ws="${this._targetNamespace}"`;
    ...
    if (hasHeaders) {
      ...
      if (hasSecurityHeaders) {
        ...
        if (this._username) {
          ...
          xml += `<wsse:Username>${this._username}</wsse:Username>`;
          xml += `<wsse:Password Type="http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0#PasswordText">${this._password}</wsse:Password>`;
    ...
    xml += `<soapenv:Body>${expandTreeToXml(this._body)}</soapenv:Body>`;
