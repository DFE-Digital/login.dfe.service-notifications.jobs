...
const sendUpdatedUserToApplication = async (action, organisation, application, config, correlationId) => {
  try {
    ...
    await secureAccessWebServiceClient.provisionOrganisation(action, establishmentDfeNumber, organisation.urn, organisation.establishmentNumber, organisation.uid, localAuthorityCode, localAuthorityName, typeId, organisation.legacyId, organisation.name, organisation.category.id, organisation.status.id, organisation.region.id, organisation.telephone);
  } catch (e) {
