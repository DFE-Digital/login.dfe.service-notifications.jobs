...
const getWebServiceClient = async (application, correlationId) => {
  ...
  const provisionUserFormatterType = application.relyingParty.params.wsProvisionUserFormatterType;
  ...
  if (!applicationClient) {
    ...
    if (provisionUserFormatterType) {
      secureAccessWebServiceClient.setProvisionUserFormatter(provisionUserFormatterType);
    }
...
const sendUpdatedUserToApplication = async (action, user, application, correlationId) => {
  try {
    ...
    await secureAccessWebServiceClient.provisionUser(action, user.legacyUserId, user.legacyUsername,
      ...
      user.organisationLACode, user.roles, user.organisationUid);
  } catch (e) {
